...
def start_cloudflare_tunnel():
    ...
    global cloudflare_tunnel_process
    try:
        import subprocess
        # Start cloudflared tunnel in a separate process
        cloudflare_tunnel_process = subprocess.Popen([
            'cloudflared', 'tunnel', '--url', 'http://127.0.0.1:20144'
        ], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        print("Cloudflare Tunnel started successfully")
    except FileNotFoundError:
        print("Error: cloudflared not found. Please install cloudflared to use Cloudflare Tunnel.")
...
def stop_cloudflare_tunnel():
    ...
    global cloudflare_tunnel_process
    if cloudflare_tunnel_process and cloudflare_tunnel_process.poll() is None:
...
def run_flask():
    # Start Cloudflare Tunnel before starting Flask
    start_cloudflare_tunnel()
    try:
        app.run(host='0.0.0.0', port=20144, debug=False, use_reloader=False)
    finally:
        # Stop Cloudflare Tunnel when Flask stops
...
async def MaiiiinE():
    ...
    Uid , Pw = '4291609832','6C0ED20B0C80BAA66778CED05264505D8EF4BAE0E8DACF99368756479E50A9C2'
